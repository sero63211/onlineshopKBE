...
public abstract class BaseClient<V> {
	...
	protected String getRequestForResponse(String endpoint, Class<String> responseType) {
		HttpEntity<String> httpEntity = getHttpEntity(null);
		return callRestTemplateUser(endpoint, HttpMethod.GET, httpEntity, responseType);
	}
	
	private String callRestTemplateUser(String endpoint, HttpMethod httpMethod, HttpEntity<?> httpEntity,
		...
		try {
			...
			ResponseEntity<String> response = getRestTemplate().exchange(uri.toUri(), httpMethod, httpEntity, responseType);
